...
int main(int argc, char *argv[])
{
	...
	io_text=new GLTexture;
	io_text->Load("data/no1.bmp");
	io_text->Use();

	io=new ImportObj("data/door2.obj");
	io->setTexture(io_text);
	io->setScalef(0.0075);
	io->setTranslatef(2,3,3);
	io->setRotatef(90,0,1,0);
	
	wall_text=new GLTexture;
	wall_text->Load("data/no1.bmp");
	wall_text->Use();

    flo->mainFloor->setTexture(wall_text);
